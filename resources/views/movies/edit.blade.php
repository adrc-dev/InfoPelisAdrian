...
@section('main')
    {{-- no hace falta verificar que tenemos user porque tenemos el middleware en la ruta --}}
        ...
        <div class="filmContainer">
            <form action="{{ route('movies.update', $movie->slug) }}" method="post" enctype="multipart/form-data">
                ...
                @error('overview')
                    <span class="error">{{ $message }}</span>
                @enderror
                <textarea name="overview" id="overview">{{ old('overview', $movie->overview) }}</textarea>

                <label for="popularity">Popularidad:</label>
                @error('popularity')
                    <span class="error">{{ $message }}</span>
                @enderror
                <input type="number" id="popularity" name="popularity" value="{{ old('popularity', $movie->popularity) }}"
                    step="0.000001">
                ...
                <input type="date" id="release_date" name="release_date"
                    value="{{ old('release_date', $movie->release_date ? $movie->release_date : '') }}">
                ...
                @error('movie_status')
                    <span class="error">{{ $message }}</span>
                @enderror
                <input type="text" id="movie_status" name="movie_status"
                    value="{{ old('movie_status', $movie->movie_status) }}">
                ...
                <input type="number" id="vote_average" name="vote_average"
                    value="{{ old('vote_average', $movie->vote_average) }}" step="0.01">
